...
from .GECL import GECLModel
from unicore.modules import LayerNorm
from typing import Optional, Dict, Any, List
from .GECL import base_architecture

logger = logging.getLogger(__name__)


@register_model("mol_confG")
class GECLConfGModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, mol_dictionary):
        super().__init__()
        GECL_confG_architecture(args)
        self.args = args
        self.GECL = GECLModel(self.args, mol_dictionary)
    ...
    def forward(
        ...
        **kwargs
        ...
        def get_dist_features(dist, et):
            n_node = dist.size(-1)
            gbf_feature = self.GECL.gbf(dist, et)
            gbf_result = self.GECL.gbf_proj(gbf_feature)
        ...
        def single_encoder(
            ...
            x = self.GECL.encoder.emb_layer_norm(emb)
            x = F.dropout(x, p=self.GECL.encoder.emb_dropout, training=self.training)
            ...
            for i in range(len(self.GECL.encoder.layers)):
                x, attn_mask, _ = self.GECL.encoder.layers[i](
                    x, padding_mask=padding_mask, attn_bias=attn_mask, return_attn=True
                )

            return x, attn_mask

        padding_mask = src_tokens.eq(self.GECL.padding_idx)
        input_padding_mask = padding_mask
        x = self.GECL.embed_tokens(src_tokens)
        ...
        if self.GECL.encoder.final_layer_norm is not None:
            x = self.GECL.encoder.final_layer_norm(x)
        ...
        if self.args.coord_loss > 0 or True:
            ...
            attn_probs = self.GECL.pair2coord_proj(delta_pair_repr)

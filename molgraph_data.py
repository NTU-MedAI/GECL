...
from module.graph_features import atom_features
...
class MolGraphDataset(data.Dataset):
    def __init__(self, path, prediction=False):
        ...
        if prediction:
            ...
            self.targets = np.genfromtxt(path, delimiter=',', skip_header=1, usecols=[0],dtype=np.int, comments=None)
...
def drug2emb_encoder(x):
    ## Sequence module parameter
    vocab_path = '/home/ntu/PycharmProjects/CY/Data/Original/Important/drug_codes_chembl.txt'
    bpe_codes_drug = codecs.open(vocab_path)
    dbpe = BPE(bpe_codes_drug, merges=-1, separator='')
    sub_csv = pd.read_csv('/home/ntu/PycharmProjects/CY/Data/Original/Important/subword_units_map_chembl.csv')
...
def molgraph_collate_fn(data):
    ...
    # return  target_tensor, d1_emb_tensor, mask_1_tensor
